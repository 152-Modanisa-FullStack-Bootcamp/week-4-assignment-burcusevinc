...
describe('Home.vue', () => {
    //home component exist check
    test('home component should exist', () => {
        API.getVideoList.mockResolvedValue([])
        const wrapper = shallowMount(Home)
        //expect(wrapper.exists()).toBe(true)
        expect(wrapper.exists()).toBeTruthy()
    })
    //video component render check
    test("video component should render correctly", async () => {
        ...
        API.getVideoList.mockResolvedValue(mockResponse) //mocked getVideoList with 3 videos
        const wrapper = shallowMount(Home) //mounted wrapper
        await flushPromises() //cleans awaiting promises
        //console.log(mockResponse)
        //find video components
        const component = wrapper.findAllComponents(Video)
        //check component length and mock response length
        expect(component).toHaveLength(mockResponse.length)
    })
})